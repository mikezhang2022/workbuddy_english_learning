...
已实现 **质量统计移动端查询页**（`/reports/quality-statistics`）：接入 `GET /api/v1/reports/quality-statistics`，按 `/me` 数据范围生成筛选选项，展示 meta / 汇总 / 明细（数量分列 + Fake 质量比率口径），处理 401/403/离线。详见 `docs/ui-quality-statistics.md`。

阶段 14：**工单进度移动端查询页**（`/reports/work-order-progress`）。详见 `docs/ui-work-order-progress.md`。

阶段 13：**生产日报移动端查询页**（`/reports/production-daily`）。详见 `docs/ui-production-daily.md`。

阶段 12：**移动 PWA 登录与应用壳**（`FactoryReport.Client`）：Cookie 会话、登录/退出、`/me` 与 dataScope 摘要展示、底部导航。计划达成与月度计划仍为「即将接入」占位。
...
- **Client 已含** 生产日报、工单进度与质量统计数据页；计划达成 / 月度计划与用户管理 UI 仍为后续阶段。**服务端后续阶段仍不包含** 正式权限分配 UI、Oracle 账号持久化、Excel 写入、MES 同步（除非另行实现）。
