...
public class QualityStatisticsReportApiTests : IClassFixture<ApiWebApplicationFactory>
{
        ...
        string? productCode = null)
    {
        ...
        return "/api/v1/reports/quality-statistics?" + string.Join("&", parts);
    }
    ...
    public async Task Get_ReturnsAggregatedRows_WithMeta()
    {
        ...
        Assert.Equal("quality_statistics", meta.GetProperty("reportCode").GetString());
        Assert.Equal("Fake", meta.GetProperty("dataAccessMode").GetString());
        Assert.True(meta.GetProperty("isFake").GetBoolean());
        Assert.Contains("Fake 测试口径", meta.GetProperty("qualityMetricsDisclaimer").GetString());
        ...
        Assert.Equal(5, row.GetProperty("defectQuantity").GetDecimal());
        Assert.Equal(3, row.GetProperty("scrapQuantity").GetDecimal());
        Assert.Equal(2, row.GetProperty("reworkQuantity").GetDecimal());
        Assert.Equal(0.9m, row.GetProperty("yieldRate").GetDecimal());
        Assert.Equal(0.05m, row.GetProperty("defectRate").GetDecimal());
    }

    [Fact]
    public async Task Get_DoesNotMerge_ScrapOrRework_IntoDefect()
    {
        var response = await _client.GetAsync(BuildUrl(
            endDate: "2026-03-10",
            productCode: DeterministicFakeFixture.ProductNormal,
    ...
    [Fact]
    public async Task Get_ZeroInspection_YieldAndDefectRateAreNull()
    {
        var response = await _client.GetAsync(BuildUrl(
            endDate: "2026-03-10",
        ...
        Assert.Equal(0, row.GetProperty("inspectionQuantity").GetDecimal());
        Assert.Equal(JsonValueKind.Null, row.GetProperty("yieldRate").ValueKind);
        Assert.Equal(JsonValueKind.Null, row.GetProperty("defectRate").ValueKind);
        Assert.Equal(1, row.GetProperty("scrapQuantity").GetDecimal());
        Assert.Equal(1, row.GetProperty("reworkQuantity").GetDecimal());
    }
