...
public class ProductionPlanAchievementReportApiTests : IClassFixture<ApiWebApplicationFactory>
{
    private readonly HttpClient _client;

    public ProductionPlanAchievementReportApiTests(ApiWebApplicationFactory factory)
    {
        ...
        string? productCode = null)
    {
        ...
        return "/api/v1/reports/production-plan-achievement?" + string.Join("&", parts);
    }

    [Fact]
    public async Task Get_NormalPlanAndActual_Calculated()
    {
        ...
        var row = Assert.Single(doc.RootElement.GetProperty("rows").EnumerateArray());
        Assert.Equal(120, row.GetProperty("planQuantity").GetDecimal());
        Assert.Equal(100, row.GetProperty("actualQuantity").GetDecimal());
        Assert.Equal(100m / 120m, row.GetProperty("achievementRate").GetDecimal());
        Assert.Equal("Calculated", row.GetProperty("planStatus").GetString());
    }

    [Fact]
    public async Task Get_PlanQuantityZero_AchievementRateNull()
    {
        var response = await _client.GetAsync(BuildUrl(
        ...
        using var doc = JsonDocument.Parse(await response.Content.ReadAsStringAsync());
        var row = Assert.Single(doc.RootElement.GetProperty("rows").EnumerateArray());
        Assert.Equal(0, row.GetProperty("planQuantity").GetDecimal());
        Assert.Equal(JsonValueKind.Null, row.GetProperty("achievementRate").ValueKind);
        Assert.Equal("PlanIsZero", row.GetProperty("planStatus").GetString());
    }

    [Fact]
    public async Task Get_ActualWithoutPlan_PlanNotConfigured_NotZeroPercent()
    {
        var response = await _client.GetAsync(BuildUrl(
        ...
        using var doc = JsonDocument.Parse(await response.Content.ReadAsStringAsync());
        var row = Assert.Single(doc.RootElement.GetProperty("rows").EnumerateArray());
        Assert.Equal(JsonValueKind.Null, row.GetProperty("planQuantity").ValueKind);
        Assert.Equal(55, row.GetProperty("actualQuantity").GetDecimal());
        Assert.Equal(JsonValueKind.Null, row.GetProperty("achievementRate").ValueKind);
        Assert.Equal("PlanNotConfigured", row.GetProperty("planStatus").GetString());
    }

    [Fact]
    public async Task Get_PlanWithoutActual_MissingActual_ZeroRate()
    {
        var response = await _client.GetAsync(BuildUrl(
        ...
        using var doc = JsonDocument.Parse(await response.Content.ReadAsStringAsync());
        var row = Assert.Single(doc.RootElement.GetProperty("rows").EnumerateArray());
        Assert.Equal(80, row.GetProperty("planQuantity").GetDecimal());
        Assert.Equal(0, row.GetProperty("actualQuantity").GetDecimal());
        Assert.Equal(0, row.GetProperty("achievementRate").GetDecimal());
        Assert.Equal("MissingActual", row.GetProperty("planStatus").GetString());
    }
    ...
    [Fact]
    public async Task Get_Filters_DateWorkshopLineProduct()
    {
        var day2 = await _client.GetAsync(BuildUrl(startDate: "2026-03-11", endDate: "2026-03-11"));
        using var dayDoc = JsonDocument.Parse(await day2.Content.ReadAsStringAsync());
        foreach (var row in dayDoc.RootElement.GetProperty("rows").EnumerateArray())
        {
    ...
    public async Task Get_Response_IncludesReportCode_AndFakeMode()
    {
        ...
        Assert.Equal("production_plan_achievement", meta.GetProperty("reportCode").GetString());
        Assert.Equal("Fake", meta.GetProperty("dataAccessMode").GetString());
        Assert.True(meta.GetProperty("isFake").GetBoolean());
        Assert.Contains("FactoryId + WorkshopId + ProductionLineId", meta.GetProperty("matchKey").GetString());
    }
}
