...
public sealed class ReportsApiClient(IHttpClientFactory httpClientFactory)
{
    ...
    public async Task<ProductionPlanAchievementQueryResponse> GetProductionPlanAchievementAsync(
        ProductionPlanAchievementClientQuery query,
        CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(query);
        var path = BuildProductionPlanAchievementPath(query);
        var client = httpClientFactory.CreateClient(ApiConstants.HttpClientName);
        HttpResponseMessage response;
        try
        {
        ...
        await ApiResponseHandler.EnsureSuccessOrThrowAsync(response, cancellationToken).ConfigureAwait(false);
        var result = await response.Content
            .ReadFromJsonAsync<ProductionPlanAchievementQueryResponse>(JsonOptions, cancellationToken)
            .ConfigureAwait(false);
        return result ?? throw new ApiRequestException(new ApiProblemDetails
        {
            Detail = "计划达成响应无效。"
    ...
    public async Task<MonthlyProductionPlanQueryResponse> GetMonthlyProductionPlanAsync(
        MonthlyProductionPlanClientQuery query,
        CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(query);
        var path = BuildMonthlyProductionPlanPath(query);
        var client = httpClientFactory.CreateClient(ApiConstants.HttpClientName);
        HttpResponseMessage response;
        try
        {
        ...
        await ApiResponseHandler.EnsureSuccessOrThrowAsync(response, cancellationToken).ConfigureAwait(false);
        var result = await response.Content
            .ReadFromJsonAsync<MonthlyProductionPlanQueryResponse>(JsonOptions, cancellationToken)
            .ConfigureAwait(false);
        return result ?? throw new ApiRequestException(new ApiProblemDetails
        {
            Detail = "月度生产计划响应无效。"
    ...
    public static string BuildProductionPlanAchievementPath(ProductionPlanAchievementClientQuery query)
    {
        var sb = new StringBuilder("/api/v1/reports/production-plan-achievement?");
        sb.Append("factoryId=").Append(query.FactoryId.ToString(CultureInfo.InvariantCulture));
        sb.Append("&startDate=").Append(query.StartDate.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture));
        sb.Append("&endDate=").Append(query.EndDate.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture));
    ...
    public static string BuildMonthlyProductionPlanPath(MonthlyProductionPlanClientQuery query)
    {
        var sb = new StringBuilder("/api/v1/reports/monthly-production-plan?");
        sb.Append("factoryId=").Append(query.FactoryId.ToString(CultureInfo.InvariantCulture));
        sb.Append("&planMonth=").Append(Uri.EscapeDataString(query.PlanMonth.Trim()));
...
/// <summary>
/// 生产计划达成客户端查询参数（与 API query 对齐）。
/// </summary>
public sealed class ProductionPlanAchievementClientQuery
{
    public long FactoryId { get; init; }
    public DateOnly StartDate { get; init; }
    public DateOnly EndDate { get; init; }
...
/// <summary>
/// 月度生产计划客户端查询参数（与 API query 对齐；PlanMonth 为 yyyy-MM）。
/// </summary>
public sealed class MonthlyProductionPlanClientQuery
{
    public long FactoryId { get; init; }
