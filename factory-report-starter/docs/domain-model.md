...
| 对象 | 职责 | 关键字段 | 关联 |
|---|---|---|---|
| `MonthlyProductionPlan` | 月度计划头 | `Id`, `FactoryId`, `PlanYearMonth`（YYYY-MM）, `PlanVersionId`, `Lines`, `CreatedAtUtc` | 对应 `monthly_production_plan` |
| `DailyProductionPlanLine` | 日计划行 | `FactoryId`, `WorkshopId`, `ProductionLineId?`, **`PlanDate`**, `ProductCode`, `PlanQuantity`, **`PlanVersionId`** | **必须保留 PlanDate**；不得由月总量均摊生成 |
