...
from ..core.theme import ACCENT, BG, BG_CARD, FG_DIM, WARN, apply_theme, center_window, f
...
class ImageTool:
    """YOLOv8 image/video import, annotate, QC, augment, export."""
    ...
    def __init__(self, ctx: AppContext, model_path: Optional[str] = None) -> None:
        ...
        self.win.title("Tool 1 — Image / Video Processing")
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(top, text="Model (.pt):", font=f(10)).pack(side=tk.LEFT)
        ...
        ttk.Button(top, text="Browse", command=self._browse_model).pack(side=tk.LEFT)
        ttk.Button(top, text="Use Default", command=self._use_default).pack(side=tk.LEFT, padx=4)
        ...
        ttk.Button(btn_row, text="Import Image", command=self._import_image).pack(side=tk.LEFT, padx=2)
        ttk.Button(btn_row, text="Import Video", command=self._import_video).pack(side=tk.LEFT, padx=2)
        ttk.Button(btn_row, text="Import Folder", command=self._import_folder).pack(side=tk.LEFT, padx=2)
        ttk.Button(btn_row, text="Auto Annotate", command=self._auto_annotate, style="Accent.TButton").pack(
            side=tk.LEFT, padx=8
        )
        ttk.Button(btn_row, text="Export Annotated Video", command=self._export_video).pack(side=tk.LEFT, padx=2)
        ...
        ttk.Label(left, text="Media List", font=f(10, "bold")).pack(anchor=tk.W)
        self.listbox = tk.Listbox(
            left, bg="#313244", fg="#cdd6f4", selectbackground="#585b70", height=20
        )
        ...
        self.preview_label = ttk.Label(center, text="Select media to preview")
        ...
        right.add(qc_frame, text="Data Health")
        ttk.Button(qc_frame, text="Run Health Check", command=self._run_qc).pack(fill=tk.X, pady=4)
        self.qc_text = tk.Text(qc_frame, height=12, bg="#313244", fg="#cdd6f4", wrap=tk.WORD)
        self.qc_text.pack(fill=tk.BOTH, expand=True)
        ttk.Button(qc_frame, text="One-Click Optimize", command=self._optimize).pack(fill=tk.X, pady=4)

        aug_frame = ttk.Frame(right, padding=8)
        right.add(aug_frame, text="Augmentation")
        self.aug_cfg = AugConfig()
        self._aug_vars = {}
        for name in ["hflip", "vflip", "rotation", "brightness", "contrast", "clahe", "sharpen", "hsv_jitter", "scale", "cutout"]:
            v = tk.BooleanVar(value=getattr(self.aug_cfg, name))
            self._aug_vars[name] = v
            ttk.Checkbutton(aug_frame, text=name, variable=v).pack(anchor=tk.W)
        ttk.Label(aug_frame, text="Multiplier:").pack(anchor=tk.W)
        self.mult_var = tk.IntVar(value=2)
        ttk.Spinbox(aug_frame, from_=1, to=10, textvariable=self.mult_var, width=6).pack(anchor=tk.W)
        self.aug_count_label = ttk.Label(aug_frame, text="Predicted new samples: 0")
        self.aug_count_label.pack(anchor=tk.W, pady=4)
        ttk.Button(aug_frame, text="Preview Augmentations", command=self._preview_aug).pack(fill=tk.X, pady=2)
        ttk.Button(aug_frame, text="Run Augmentation", command=self._run_aug).pack(fill=tk.X, pady=2)

        export_frame = ttk.Frame(right, padding=8)
        right.add(export_frame, text="Export")
        ttk.Label(export_frame, text="Min images threshold:").pack(anchor=tk.W)
        ...
        ttk.Button(export_frame, text="Export train/val + data.yaml", command=self._export_dataset).pack(
            fill=tk.X, pady=8
        )
        ttk.Button(
            export_frame, text="Send to Training Tool →", command=self._send_to_train, style="Accent.TButton"
        ).pack(fill=tk.X)
    ...
    def _update_model_status(self) -> None:
        if self.model_path:
            self.model_status.config(text="Custom model", foreground=ACCENT)
        else:
            self.model_status.config(
                text=f"Default: {DEFAULT_MODEL} (COCO classes)", foreground=WARN
            )
    ...
    def _refresh_list(self) -> None:
        ...
        for it in self.items:
            self.listbox.insert(tk.END, f"[{it.kind}] {it.name}")
    ...
    def _auto_annotate(self) -> None:
        def work():
            ...
            for i, item in enumerate(self.items):
                self._set_status(f"Annotating {i+1}/{len(self.items)}: {item.name}")
                if item.kind == "image":
                    img = imread_unicode(item.path)
                    if img is None:
                        continue
            ...
            self._set_status("Annotation complete")
    ...
    def _export_video(self) -> None:
        sel = self.listbox.curselection()
        if not sel:
            messagebox.showwarning("Export", "Select a video first")
            return
        item = self.items[sel[0]]
        if item.kind != "video":
            messagebox.showwarning("Export", "Selected item is not a video")
            return
        out = filedialog.asksaveasfilename(defaultextension=".mp4", filetypes=[("MP4", "*.mp4")])

        def work():
            self._set_status("Exporting annotated video...")
            self.predictor.load(self.model_path or None)
            ok = annotate_video(item.path, out, self.predictor)
            self._set_status("Video exported" if ok else "Export failed")
    ...
    def _run_qc(self) -> None:
        def work():
            ...
            self._set_status("Health check done")

        threading.Thread(target=work, daemon=True).start()

    def _optimize(self) -> None:
        if not hasattr(self, "_last_report"):
            messagebox.showinfo("Optimize", "Run health check first")
            return

        def work():
            n = apply_fixes(self._last_report, self.images_dir, self.labels_dir)
            self._set_status(f"Applied {n} fixes")
            self._run_qc()
    ...
    def _update_aug_estimate(self) -> None:
        ...
        self.aug_count_label.config(text=f"Predicted new samples: {est}")

    def _preview_aug(self) -> None:
        imgs = list(self.images_dir.glob("*"))
        if not imgs:
            messagebox.showinfo("Preview", "No images in dataset folder")
            return
        ...
        # show first in preview
        if samples:
            photo = _cv2_to_tk(samples[0])
            self._photos = [photo]
            self.preview_label.config(image=photo, text="Augmentation preview (1 of 4)")
    ...
    def _run_aug(self) -> None:
        ...
        def work():
            self._set_status("Running augmentation...")
            n = augment_run(self.images_dir, self.labels_dir, cfg, mult)
            self._set_status(f"Generated {n} new samples")
            self._update_aug_estimate()
            self._check_insufficient_data()
    ...
    def _check_insufficient_data(self) -> None:
        ...
        if n < threshold:
            msg = (
                f"Dataset has {n} images (< {threshold}).\n"
                "Recommended: enable augmentation in the Augmentation tab and export when ready."
            )
            self.win.after(0, lambda: messagebox.showinfo("Insufficient Data", msg))

    def _export_dataset(self) -> None:
        imgs = [p for p in self.images_dir.iterdir() if p.is_file()]
        if not imgs:
            messagebox.showwarning("Export", "No images in dataset")
            return
        random.shuffle(imgs)
        ...
        messagebox.showinfo("Export", f"Dataset exported to:\n{yaml_path}")
        self._set_status(f"Exported {len(train_imgs)} train, {len(val_imgs)} val")
